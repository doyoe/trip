...
因为大部分浏览器的默认字号都是 `16px`，所以 `html` 的字号计算出来应该是 `16px * 62.5% = 10px`。此时，我们预期 `body` 的 `font-size` 为 `14px`。然而实际情况与我们想象的不太一样，最终 `body` 的计算值并不是 `14px`，它直接忽略了 `html` 的定义，而是直接使用了浏览器的默认字号作为参照。
